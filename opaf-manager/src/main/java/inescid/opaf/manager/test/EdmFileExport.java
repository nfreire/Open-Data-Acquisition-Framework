...
public class EdmFileExport {
		...
		public void close() throws IOException {
			if(fileOutputStream!=null) {
				IOUtils.write("</rdf:RDF>".getBytes(EdmFromRepositoryToFileExport.UTF8), fileOutputStream);
...
//			System.out.println(new String(contentBytes, UTF8));
			...
			Matcher matcher = EdmFromRepositoryToFileExport.STRIP_XML_TOP_ELEMENT.matcher(contentStr);
			if(!matcher.find()) 
			...
			if(fileOutputStream==null) {
				...
				IOUtils.write(rdfDecl.getBytes(EdmFromRepositoryToFileExport.UTF8), fileOutputStream);
			}
			IOUtils.write("<ore:aggregates>\n".getBytes(EdmFromRepositoryToFileExport.UTF8), fileOutputStream);
			IOUtils.write(contentStr.getBytes(EdmFromRepositoryToFileExport.UTF8), fileOutputStream);
			IOUtils.write("</ore:aggregates>\n".getBytes(EdmFromRepositoryToFileExport.UTF8), fileOutputStream);
