...
public class EdmFromRepositoryToFileExport {
...
//		boolean transformToEdmInternal=true;
		...
		Database db;
//		db=new Database(repositoryFolder, AccessMode.WRITE);
		...
		DataConverter dcToEdmConverter = DataConversionManager.getInstance().getConverter(null, null);
...
//							System.out.println(seeAlso.getProfile());
					...
					for(RawDataRecord seeAlso: manifest.getMetadata().getSeeAlso()) {
						if(seeAlso.getProfile()!=null && seeAlso.getProfile().equalsIgnoreCase("http://www.europeana.eu/schemas/edm/")) {
							...
							RawDataRecord convertedEdm = dcToEdmConverter.convert(seeAlso, getEdmStatements(manifest, provider, dataProvider));
